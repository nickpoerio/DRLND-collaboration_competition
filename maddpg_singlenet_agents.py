...
from maddpg_singlenet_model import Actor, Critic
...
class Agent():
    ...
    def learn(self):
        ...
        actions_next = torch.from_numpy(np.zeros((BATCH_SIZE, a_size*n_agents))).float().to(device) #initialize actions to 0
        actions_pred = torch.from_numpy(np.zeros((BATCH_SIZE, a_size*n_agents))).float().to(device) #initialize actions to 0
